/**
 * Deterministic seeded RNG + 2D simplex noise + fbm.
 * Everything procedural in LifeVerse must flow from these (worldSeed + id),
 * so the same seed always rebuilds the same world.
 */

/** Mulberry32 — tiny fast seeded PRNG, [0,1). */
...
/** Derive a sub-seed from a parent seed + string id (stable across sessions). */
export function seedFrom(seed: number, id: string): number {
  let h = seed >>> 0;
...
/** Fractal Brownian motion over a Noise2D. Returns roughly [-1, 1]. */
